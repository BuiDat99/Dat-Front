...
export default function Papers() {
    ...
    const inputNamePaperRef = React.createRef();
    const inputTimePaperRef = React.createRef();
    const inputNumberPaperRef = React.createRef();
    ...
    // }
    useEffect(() => {
        searchPaper({ page: 1, limit: rowsPerPage });
    ...
    const searchPaper = (dataSearch = {}) => {

        makeRequest('get', `paper/search`, dataSearch)
            .then(({ data }) => {
                if (data.signal) {
                    const res = data.data.ListSearchPaper;
                    setRow(res);
    ...
    const clickModalOk = () => {
        ...
        makeRequest('get', `paper/delete?id=${idDel}`)
            .then(({ data }) => {
                if (data.signal) {
                    ...
                    return showErrorMessage('Please, delete a question have this paper first');
                }
            })
            .catch(err => {
    ...
    const handleChangePage = (newPage) => {
        setPage(newPage);
        if (isLoad) {
            searchPaper({ ...dataSearch, page: newPage, limit: rowsPerPage });
            return;
        }
        searchPaper({ page: newPage, limit: rowsPerPage });
        return;
    };
    const handleSubmit = (e) => {
        e.preventDefault();
        searchPaper(dataSearch, setPage(1));
        setLoad(true);
    }
    ...
    const handleSubmitAdd = (e) => {
        e.preventDefault();
        if (!dataAdd.name) {
            inputNamePaperRef.current.focus();
            return showErrorMessage('please enter paper name');
        }
        if (!dataAdd.time || dataAdd.time <= 0) {
            inputTimePaperRef.current.focus();
            return showErrorMessage('please enter paper time ');
        }
        if (!dataAdd.questionNumber || dataAdd.questionNumber <= 0) {
            inputNumberPaperRef.current.focus();
            return showErrorMessage('please enter paper question number ');
        }

        //enableLoadSubmit();
        makeRequest('post', `paper/create`, dataAdd)
            .then(({ data }) => {
                if (data.signal) {
    ...
    const handleSubmitUpdate = (e) => {
        ...
        if (!dataUpdate.time || dataUpdate.time <= 0) {
            inputTimePaperRef.current.focus();
            return showErrorMessage('please enter paper time');
        }
        if (!dataUpdate.questionNumber || dataUpdate.questionNumber <= 0) {
            inputNumberPaperRef.current.focus();
            return showErrorMessage('please enter paper question number');
        }
        makeRequest('post', `paper/update`, dataUpdate)
            .then(({ data }) => {
                if (data.signal) {
    ...
    return (
        <>
            <Link onClick={showModalAdd} Icon="" id="tooltip" className="btn btn-primary btn-bold btn-sm btn-icon-h kt-margin-l-10">Add Paper
            </Link>
            <div className="row">
                <div className="col-md-12">
                    <div className="kt-section">
                        ...
                        <Modal
                            ...
                            okText='Ok'
                        >
                            <p>Do you want to delete this paper?</p>
                        </Modal>
                        <Modal
                            title='Add New Paper'
                                                ...
                                                <Form onSubmit={handleSubmitAdd}>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            ...
                                                            <Form.Control type="text" autoFocus maxLength={255} ref={inputNamePaperRef} placeholder="Enter Name Paper" value={dataAdd.name || ''} onChange={(e) => onChangeAddValue('name', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Time</Form.Label>
                                                            <Form.Control type="number" min="0" maxLength={255} ref={inputTimePaperRef} placeholder="Enter Time Paper" value={dataAdd.time || ''} onChange={(e) => onChangeAddValue('time', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Question Number</Form.Label>
                                                            <Form.Control type="number" min="0" maxLength={255} ref={inputNumberPaperRef} placeholder="Enter Question Number Paper" value={dataAdd.questionNumber || ''} onChange={(e) => onChangeAddValue('questionNumber', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col}>
                                            ...
                                            <Card.Body>
                                                <Form onSubmit={handleSubmitUpdate}>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Name</Form.Label>
                                                            <Form.Control type="text" autoFocus maxLength={255} ref={inputNamePaperRef} placeholder="Enter name paper" value={dataUpdate.name || ''} onChange={(e) => onChangeUpdateValue('name', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Time</Form.Label>
                                                            <Form.Control type="number" min="0" autoFocus maxLength={255} ref={inputTimePaperRef} placeholder="Enter time paper" value={dataUpdate.time || ''} onChange={(e) => onChangeUpdateValue('time', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Question Number</Form.Label>
                                                            <Form.Control type="number" min="0" autoFocus maxLength={255} ref={inputNumberPaperRef} placeholder="Enter question number paper" value={dataUpdate.questionNumber || ''} onChange={(e) => onChangeUpdateValue('questionNumber', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label>Description</Form.Label>
                                                            <Form.Control type="text" as="textarea" rows="5" maxLength={255} ref={inputDescriptionRef} placeholder="Enter description" value={dataUpdate.description || ''} onChange={(e) => onChangeUpdateValue('description', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                            ...
                                            <Card.Body>
                                                <Form>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Name</Form.Label>
                                                            <Form.Control readOnly type="text" maxLength={255} placeholder="Enter name paper" value={dataDetail.name || ''} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Time</Form.Label>
                                                            <Form.Control readOnly type="text" maxLength={255} placeholder="Enter name paper" value={dataDetail.time || ''} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Question Number</Form.Label>
                                                            <Form.Control readOnly type="text" maxLength={255} placeholder="Enter name paper" value={dataDetail.questionNumber || ''} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col}>
