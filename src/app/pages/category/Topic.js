...
import ListRole from 'list_role4';
...
export default function Topic(props) {
    ...
    const inputNameTopicRef = React.createRef();
    ...
    useEffect(() => {
        searchTopic({ page: 1, limit: rowsPerPage });
    ...
    const searchTopic = (dataSearch = {}) => {
        makeRequest('get', `topic/search`, dataSearch)
            .then(({ data }) => {
                if (data.signal) {
                    const res = data.data.ListSearchTopic;
                    setRow(res);
    ...
    const clickModalOk = () => {
        ...
        makeRequest('get', `topic/delete?id=${idDel}`)
            .then(({ data }) => {
                if (data.signal) {
                    ...
                    return showErrorMessage('Please, delete a question have this topic first');
                }
            })
            .catch(err => {
    ...
    const handleChangePage = (newPage) => {
        setPage(newPage);
        if (isLoad) {
            searchTopic({ ...dataSearch, page: newPage, limit: rowsPerPage });
            return;
        }
        searchTopic({ page: newPage, limit: rowsPerPage });
        return;
    };
    const handleSubmit = (e) => {
        e.preventDefault();
        searchTopic(dataSearch, setPage(1));
        setLoad(true);
    }
    ...
    const handleSubmitAdd = (e) => {
        e.preventDefault();
        if (!dataAdd.name) {
            inputNameTopicRef.current.focus();
            return showErrorMessage('please enter topic name');
        }


        //enableLoadSubmit();
        makeRequest('post', `topic/create`, dataAdd)
            .then(({ data }) => {
                if (data.signal) {
    ...
    const handleSubmitUpdate = (e) => {
        ...
        makeRequest('post', `topic/update`, dataUpdate)
            .then(({ data }) => {
                if (data.signal) {
    ...
    return (
        <>
            ...
            <Link onClick={showModalAdd} Icon="" id="tooltip" className="btn btn-primary btn-bold btn-sm btn-icon-h kt-margin-l-10">Add Topic
            </Link>
            <div className="row">
                        ...
                        <div className="kt-section__content">
                            <Paper className={classes1.root}>
                                ...
                                <ListRole
                                    ...
                                    showModal={showModal} showModalDetail={showModalDetail} showModalUpdate={showModalUpdate}
                                    ...
                                    dataDetail={dataDetail} clickModalDetailCancel={clickModalDetailCancel}
                                />
                            </Paper>
                            {total > rowsPerPage && (
                                                ...
                                                <Form onSubmit={handleSubmitAdd}>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            ...
                                                            <Form.Control type="text" autoFocus maxLength={255} ref={inputNameTopicRef} placeholder="Enter Name Topic" value={dataAdd.name || ''} onChange={(e) => onChangeAddValue('name', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col}>
                                            ...
                                            <Card.Body>
                                                <Form onSubmit={handleSubmitUpdate}>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Name</Form.Label>
                                                            <Form.Control type="text" autoFocus maxLength={255} ref={inputNameTopicRef} placeholder="Enter name topic" value={dataUpdate.name || ''} onChange={(e) => onChangeUpdateValue('name', e.target.value)} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                            ...
                                            <Card.Body>
                                                <Form>
                                                    <Form.Row>
                                                        <Form.Group as={Col} controlId="formBasicNameBank">
                                                            <Form.Label className="starDanger">Name</Form.Label>
                                                            <Form.Control readOnly type="text" maxLength={255} placeholder="Enter name topic" value={dataDetail.name || ''} />
                                                        </Form.Group>
                                                    </Form.Row>
                                                    <Form.Row>
                                                        <Form.Group as={Col}>
